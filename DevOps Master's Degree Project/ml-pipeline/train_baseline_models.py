...
class BaselineModelTrainer:
    ...
    def load_time_series_data(self, time_series_file: Path) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        # Drop metadata columns (including synthetic data markers)
        metadata_cols = ['date', 'branch', 'label', 'timestamp', 'synthetic', 'source']
        X = df.drop(metadata_cols, axis=1, errors='ignore')
        y = df['label'].values
    ...
    def preprocess_data(self, X: pd.DataFrame, y: np.ndarray, test_size: float = 0.2) -> Dict:
        ...
        if min_class_count < 2:
            print(f"\n[!] Warning: Very small dataset ({len(y)} samples)")
            print(f"    Smallest class has only {min_class_count} sample(s)")
            print(f"    Using simple split (no stratification)")
        ...
        print(f"\n[OK] Data scaled using StandardScaler")
    ...
    def train_random_forest(self, data: Dict) -> Dict:
        ...
        if len(data['y_train']) >= 5:
            try:
                cv_scores = cross_val_score(
                    model, data['X_train'], data['y_train'],
                    cv=StratifiedKFold(n_splits=min(5, len(data['y_train'])), shuffle=True, random_state=42),
                    scoring='accuracy',
                    n_jobs=-1
                )
                ...
                print("  [Skip] Cross-validation (dataset too small)")
        ...
        if results['cv_accuracy_mean'] is not None:
    ...
    def plot_feature_importance(self, top_n: int = 20):
        ...
        print(f"\n[OK] Feature importance plot saved: {output_file.name}")
        plt.close()
    ...
    def plot_confusion_matrices(self):
        ...
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        print(f"[OK] Confusion matrices saved: {output_file.name}")
        plt.close()
    ...
    def save_models(self):
        ...
        print(f"[OK] Saved: {results_file.name}")
    ...
    def generate_report(self):
        ...
        print(f"\n[OK] Training report saved: {report_file.name}")
